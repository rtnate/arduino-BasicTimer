...
class BasicTimerCallbackUnitTests: public testing::Test {
    protected:
        void SetUp() override {
            GetTime()->reset();
            cbInstance_ = this;
    ...
    private:
        static BasicTimerCallbackUnitTests* cbInstance_;
...
BasicTimerCallbackUnitTests* BasicTimerCallbackUnitTests::cbInstance_ = nullptr;

TEST_F(BasicTimerCallbackUnitTests, TestCallbackInvokable) {
...
TEST_F(BasicTimerCallbackUnitTests, TestBasicTimerExpireCallback) {
    ASSERT_EQ(CallbackInvokedCount(), 0);
    ASSERT_FALSE(CallbackWasInvoked());
    ASSERT_EQ(LastCallbackTimestamp(), 0);
    ...
    EXPECT_EQ(CallbackInvokedCount(), 1);
    EXPECT_TRUE(CallbackWasInvoked());
    EXPECT_EQ(LastCallbackTimestamp(), 501);
}

TEST_F(BasicTimerCallbackUnitTests, TestBasicTimerMultipleExpireCallback) {
    ASSERT_EQ(CallbackInvokedCount(), 0);
    ASSERT_FALSE(CallbackWasInvoked());
    ASSERT_EQ(LastCallbackTimestamp(), 0);
...
TEST_F(BasicTimerCallbackUnitTests, TestBasicTimerExpireCallbackWithIntArgument) {
    ASSERT_EQ(CallbackInvokedCount(), 0);
    ASSERT_FALSE(CallbackWasInvoked());
    ASSERT_EQ(LastCallbackTimestamp(), 0);
    ASSERT_EQ(LastCallbackIntArgument(), -1);

    BasicTimer timer(800);
...
TEST_F(BasicTimerCallbackUnitTests, TestBasicTimerExpireCallbackWithPointerArgument) {
    ASSERT_EQ(CallbackInvokedCount(), 0);
    ASSERT_FALSE(CallbackWasInvoked());
    ASSERT_EQ(LastCallbackTimestamp(), 0);
    ASSERT_EQ(LastCallbackIntArgument(), -1);

    BasicTimer timer(999);
